...
defineFeature(feature, (test) => {
  ...
  test('Customer selects a button from the carousel', ({
    given,
    when,
    and,
    then,
    ...
    when('the customer receives a carousel message', () => {
      ...
      expect(component.handleCimMessage).toHaveBeenCalledWith(
        carousalMessage.data,
      );
    });
    ...
    then(
      'the selected button should be shown as a quoted reply in the conversation',
      () => {
        ...
        const lastMessage =
          component.cimMessage[component.cimMessage.length - 1];
        ...
        expect(lastMessage.body.quotedCardImage).toBe(
  ...
  test('Customer selects a list from the carousel', ({
    given,
    when,
    and,
    then,
  ...
  test('Successful carousel response with clickable URL', ({
    given,
    when,
    then,
    and,
    ...
    when(
      'the customer receives a carousel message from the bot with a card containing a URL',
      () => {
        carousalMessage = {
          type: 'MESSAGE_RECEIVED',
          data: {
  ...
  test('Customer attempts to respond to carousel after submitting', ({
    given,
    when,
    then,
    and,
    ...
    and(
      'the quoted reply should be visually associated with the carousel card',
      () => {
        ...
        expect(quotedMessage.body.quotedCardImage).toBe(
  ...
  test('Customer refreshes the browser after submits the carousel response', ({
    given,
    when,
    then,
  }) => {
    ...
    then(
      'the submitted carousel response should be displayed as non interactive',
      () => {
        expect(component.handleCarousalQuotedMessage).toHaveBeenLastCalledWith(
          cimMessage,
        );
      },
    );
