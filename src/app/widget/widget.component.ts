...
export class WidgetComponent implements OnInit, AfterViewInit {
    ...
    this.widgetConfigsSubscription = this.sdk.widgetConfigs$.subscribe(
      (configs: any) => {
        ...
        if (this.audioStart) {
          ...
          this.initializeAutoStartAudioCall();
        } else {
  ...
  private initializeAutoStartAudioCall(): void {
    // Verify WebRTC config is loaded before starting auto audio call
    if (!this.webRTCConfig || !this.enableWebRtc) {
      console.warn('WebRTC not enabled. Config:', this.webRTCConfig, 'Enabled:', this.enableWebRtc);
      return;
    }

    const autoGeneratedIdentifier = 'auto_' + Math.floor(10000000 + Math.random() * 90000000).toString();

    this.customerData = {
      serviceIdentifier: this.serviceIdentifier,
      channelCustomerIdentifier: autoGeneratedIdentifier,
      browserDeviceInfo: {
  ...
  private handleChannelSessionStarted(event: any) {
    ...
    // Auto-start audio call if audioStart parameter is set
    if (this.audioStart) {
      console.log('Initiating auto-start audio call...');
      setTimeout(() => {
        this.subscribeToStreams();
